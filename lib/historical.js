...
var $ = require("jquery"); // var returnPeriods=require('./getReturnPeriods.js');
...
var units;
var returnShapes;
var endpoint = "http://0.0.0.0:8090/api/"; //** THIS FUNCTIONS RETRIEVES THE HISTORICAL DATA IN A GRAPH **//

module.exports = {
  graph_h: function (reachid, htmlELement, width, height) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        flow = response_timeSeries['flow'];
        units = data['units']['short']; // var allLines = data.split('\n');
      ...
      complete: function () {
        ...
        var layout = {
          ...
          yaxis: {
            title: units,
            range: [0, Math.max(...data_array[0].y) + Math.max(...data_array[0].y) / 5],
            showgrid: false
          } // plot_bgcolor:"#7782c5",
          //shapes: returnShapes,
        ...
        Plotly.newPlot(htmlELement, data_array, layout); // console.log("COMPLETE PART OF Streamflow()");
        // var mean = {
        //     name: 'Mean',
        //     x: dates.dates,
        //     y: values.mean,
