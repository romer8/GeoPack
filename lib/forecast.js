...
var returnPeriods = require('./returnPeriods.js'); //**GLOBAL VARIABLES TO DEAL WITH THE FUNCTIONS**//


const ENPOINT = "https://tethys2.byu.edu/localsptapi/api/"; //** THIS FUNCTIONS RETRIEVES THE FORECAST DATA IN A GRAPH **//
...
module.exports = {
  ...
  graph_fr: function (reachid, htmlElement, title, rp, width, height) {
    ...
    $.ajax({
      ...
      complete: function () {
        ...
        if (rp) {} // returnPeriods.graph_rp(reachid,htmlElement,width,height);
  ...
  graph_emsembles: function (reachid, htmlElement, arrayEnsemble, title, width, height) {
    ...
    height = typeof heigth !== 'undefined' ? heigth : 500;
    title = typeof title !== 'undefined' ? title : 'Reach ID: ' + reachid;
    arrayEnsemble = typeof arrayEnsemble !== 'undefined' ? arrayEnsemble : Array.from(Array(52).keys()).map(function (value) {
      return value + 1;
    });
    var dates = {
      highres: [],
    ...
    $.ajax({
      ...
      success: function (data) {
        var response_timeSeries = data['time_series'];
        var dates_prep = response_timeSeries['datetime'];
        var dates_prep_high_res = response_timeSeries['datetime_high_res']; // Pushing the dates for the normal ensembles //
      ...
      complete: function () {
        ...
        var showLegendPromp = "";
        var fillPromp = "none"; //making the trace object for all the emsembles //
        ...
        values_emsembles_keys.forEach(function (x) {
          ...
          if (arrayEnsemble.includes(numberEnsemble)) {
            ...
            if (x !== "ensemble_52_m^3/s") {
              ...
              data.push(singleEmsemble); // add data to download //
      ...
      success: function (data) {
        ...
        $.ajax({
          ...
          success: function (resp) {
            ...
            dataPlot.push(mean);
            dataPlot.push(highres); // Download//
